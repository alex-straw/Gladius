...
def prepare_spreadsheets(df):
    ...
    buy_df['price_x_size'] = buy_df['Token price USD'] * buy_df['size']  # Vectorised prior to loop to minimise processing done within for loop                 
    buy_df.at[0,'cost_basis'] = buy_df['Token price USD'].values[0]  # Set up the first cost_basis --> equal to the very first purchase price                     
    ...
    buy_df = df.drop('price_x_size', 1) # Drop temporary calculation column prior to merge            
